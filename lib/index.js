...
function prepareBatch(session, tokenMeter, config, state, args, firstBlockNumber) {
	...
	for (const entry of args.content) {
		const result = resolveRange([...session.surface.nodes], state.boundaryRefs, entry.startRef, entry.endRef);
		if (!result.ok) return {
			ok: false,
...
*/
function cleanupInlineSummary(session, tokenMeter, compressCallId, blockRefs) {
	let authorSeq;
	let original;
...
function executeCompressRange(session, tokenMeter, config, args, meta) {
	...
	const cleanup = blocks.length > 0 ? cleanupInlineSummary(session, tokenMeter, meta.compressCallId, blocks.map((block) => block.blockRef)) : {
		cleaned: false,
...
function buildStepMarkerMessage(ref, turn, step, nudgeText) {
...
function computeNudge(session, measure, config) {
	...
	if (!config.nudge.enabled || contextWindow === void 0) return {
		armed,
		stepsSinceNudge
	};
	const ratio = measure.totalTokens / contextWindow;
	if (ratio > config.nudge.maxRatio && armed && stepsSinceNudge >= config.nudge.frequencySteps) return {
	...
	ctx.on("agent/pre-step", async ({ agent, messages, turn, step, signal }, next) => {
		...
		if (messages.some(isDcpControlMessage)) {
			for (const message of messages) {
				...
				if (control.kind === "expand" && control.arg) applyExpansion(agent.session, ctx.tokenMeter, control.arg);
				if (control.kind === "recompress" && control.arg) applyRecompress(agent.session, ctx.tokenMeter, control.arg);
			}
			return {
				kind: "enter",
				messages: []
		...
		if (decision.kind === "enter") {
			...
			return {
				kind: "enter",
				messages: [buildStepMarkerMessage(`m${String(currentState.maxMarkerNumber + 1).padStart(4, "0")}`, turn, step, computeNudge(agent.session, ctx.tokenMeter.measure(agent.session), resolved).text), ...decision.messages]
			};
		}
		return decision;
	});
