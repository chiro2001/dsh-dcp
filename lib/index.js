...
;(function (Binary) {
  ...
  function toHex(source) {
    ...
    return Array.from(new Uint8Array(source), (byte) =>
...
function deepEqual(a, b, strict) {
  ...
  return (
    check(
      Array.isArray,
      (a, b) => a.length === b.length && a.every((item, index) => deepEqual(item, b[index])),
    ) ??
...
;(function (Time) {
  ...
  function parseTime(source) {
    ...
    if (!capture) return 0
    return (
      (parseFloat(capture[1]) * Time.week || 0) +
...
defineMethod(
  'dict',
  ['inner', 'sKey'],
  ({ inner, sKey }) => `{ [key: ${sKey.toString()}]: ${inner.toString()} }`,
)
defineMethod(
  'tuple',
  ['list'],
  ({ list }) => `[${list.map((inner) => inner.toString()).join(', ')}]`,
)
...
defineMethod('transform', ['inner', 'callback', 'preserve'], ({ inner }, isInner) =>
  inner.toString(isInner),
)
//#endregion
...
const Config = Schema.object({
  ...
  compress: Schema.object({
    ...
    protectUserMessages: Schema.boolean().default(
      DCP_CONFIG_DEFAULTS.compress.protectUserMessages,
    ),
  ...
  references: Schema.object({
    transport: Schema.union([Schema.const('marker'), Schema.const('context-tool')]).default(
      DCP_CONFIG_DEFAULTS.references.transport,
    ),
  ...
  manualMode: Schema.object({
    ...
    automaticStrategies: Schema.boolean().default(
      DCP_CONFIG_DEFAULTS.manualMode.automaticStrategies,
    ),
  ...
  subagents: Schema.object({
    enableCompressionInChild: Schema.boolean().default(
      DCP_CONFIG_DEFAULTS.subagents.enableCompressionInChild,
    ),
...
function collectKeyPaths(value, prefix = '') {
  ...
  for (const key of Object.keys(value)) {
    ...
    if (child !== null && typeof child === 'object' && !Array.isArray(child))
      keys.push(...collectKeyPaths(child, full))
  }
  return keys
}
...
function decodeDcpMeta(source) {
  ...
  if (meta.endRef !== void 0 && (typeof meta.endRef !== 'string' || meta.endRef.length === 0))
    return {
      ok: false,
      diagnostic: {
  ...
  if (meta.recompressedFrom !== void 0 && !isBlockRef(meta.recompressedFrom))
    return {
      ok: false,
      diagnostic: {
...
function validateCompressArgs(args, maxRanges) {
  ...
  for (const [index, entry] of record.content.entries()) {
    ...
    else if (range.summary.length > 8e3)
...
function prepareRange(session, tokenMeter, config, state, entry, blockRef, topic) {
  ...
  const turns = shadowedSeqs
    ...
    .filter((turn) => turn !== void 0)
  const latestTurn = Math.max(0, ...surface.map((seq) => turnOfSeq(events, seq) ?? 0))
  if (Math.max(0, ...turns) > latestTurn - config.compress.retainRecentTurns)
    return {
      ok: false,
      errors: [
...
function classifyCompactionPrefix(events) {
  ...
  if (bracket.endSeq !== void 0)
    return bracket.replaceSeq === void 0 ? 'failed-attempt' : 'committed'
...
function cleanupInlineSummary(session, tokenMeter, compressCallId, blockRef) {
  ...
  if (authorSeq === void 0 || original?.type !== 'assistant/message')
    return {
      cleaned: false,
...
function createCompressTool(ctx, config) {
  return defineTool({
    name: 'compress',
    description:
      'Compress one closed conversation range into a summary checkpoint. Ranges are half-open [startRef, endRef): startRef is included, endRef is excluded. Use boundary refs visible in context (mNNNN). Only closed, tool-pairing-balanced ranges are accepted.',
    parameters: {
      topic: {
        type: 'string',
        required: true,
...
//#region src/index.ts
const name = 'dsh-dcp'
const inject = ['sessions', 'tokenMeter', 'systemPrompt', 'tools', 'commands']
