...
declare namespace StandardSchemaV1 {
  ...
  interface Props<Input = unknown, Output = Input> extends StandardTypedV1.Props<
    Input,
    Output
  > {
  ...
  interface Types<Input = unknown, Output = Input> extends StandardTypedV1.Types<
    Input,
    Output
  > {}
...
declare global {
  namespace Schemastery {
    ...
    type From<X> = X extends string | number | boolean
      ? Schema<X>
      : X extends Schema
        ? X
        : X extends typeof String
          ? Schema<string>
          : X extends typeof Number
            ? Schema<number>
            : X extends typeof Boolean
              ? Schema<boolean>
              : X extends typeof Function
                ? Schema<Function, (...args: any[]) => any>
                : X extends Constructor<infer S>
                  ? Schema<S>
                  : never
    ...
    type TupleS<X extends readonly any[]> = X extends readonly [infer L, ...infer R]
      ? [TypeS<L>?, ...TupleS<R>]
      : any[]
    type TupleT<X extends readonly any[]> = X extends readonly [infer L, ...infer R]
      ? [TypeT<L>?, ...TupleT<R>]
      : any[]
    ...
    interface Static {
      ...
      bitset<K extends string>(
        bits: Partial<Record<K, number>>,
      ): Schema<number | readonly K[], number>
      /** Accept functions. */
      ...
      dict<X, Y extends Schema<any, string> = Schema<string>>(
        inner: X,
        sKey?: Y,
      ): Schema<Dict<TypeS<X>, TypeS<Y>>, Dict<TypeT<X>, TypeT<Y>>>
      ...
      transform<X, T>(
        inner: X,
        callback: (value: TypeS<X>, options: Schemastery.Options) => T,
        preserve?: boolean,
      ): Schema<TypeS<X>, T>
...
interface DcpReferencesConfig {
  transport: 'marker' | 'context-tool'
  maxAliasEntries: number
  excerptChars: number
}
interface DcpNudgeConfig {
  enabled: boolean
  maxRatio: number
  minRatio: number
...
interface DcpManualModeConfig {
  default: boolean
  automaticStrategies: boolean
}
interface DcpDeduplicationConfig {
  enabled: boolean
  protectedTools: string[]
}
interface DcpPurgeErrorsConfig {
  enabled: boolean
  turns: number
  protectedTools: string[]
}
interface DcpStrategiesConfig {
  deduplication: DcpDeduplicationConfig
  purgeErrors: DcpPurgeErrorsConfig
}
interface DcpSubagentsConfig {
...
declare const Config: Schema<
  Schemastery.ObjectS<{
    ...
    compress: Schema<
      Schemastery.ObjectS<{
        enabled: Schema<boolean, boolean>
        mode: Schema<'range', 'range'>
        maxRangesPerCall: Schema<number, number>
        minNetSavingsTokens: Schema<number, number>
        retainRecentTurns: Schema<number, number>
        protectUserMessages: Schema<boolean, boolean>
        protectTags: Schema<boolean, boolean>
        protectedTools: Schema<string[], string[]>
        protectedSources: Schema<string[], string[]>
      }>,
      Schemastery.ObjectT<{
        enabled: Schema<boolean, boolean>
        mode: Schema<'range', 'range'>
    ...
    references: Schema<
      Schemastery.ObjectS<{
        transport: Schema<'context-tool' | 'marker', 'context-tool' | 'marker'>
        maxAliasEntries: Schema<number, number>
        excerptChars: Schema<number, number>
      }>,
      Schemastery.ObjectT<{
        transport: Schema<'context-tool' | 'marker', 'context-tool' | 'marker'>
        maxAliasEntries: Schema<number, number>
    ...
    nudge: Schema<
      Schemastery.ObjectS<{
        enabled: Schema<boolean, boolean>
        maxRatio: Schema<number, number>
        minRatio: Schema<number, number>
        frequencySteps: Schema<number, number>
        iterationThreshold: Schema<number, number>
      }>,
      Schemastery.ObjectT<{
        enabled: Schema<boolean, boolean>
        maxRatio: Schema<number, number>
    ...
    manualMode: Schema<
      Schemastery.ObjectS<{
        ...
        automaticStrategies: Schema<boolean, boolean>
      }>,
      Schemastery.ObjectT<{
        default: Schema<boolean, boolean>
        automaticStrategies: Schema<boolean, boolean>
    ...
    >
    protectedFilePatterns: Schema<string[], string[]>
    subagents: Schema<
      Schemastery.ObjectS<{
        enableCompressionInChild: Schema<boolean, boolean>
        readChildSession: Schema<boolean, boolean>
      }>,
      Schemastery.ObjectT<{
        enableCompressionInChild: Schema<boolean, boolean>
        readChildSession: Schema<boolean, boolean>
...
declare const name = 'dsh-dcp'
declare const inject: string[]
declare function apply(ctx: Context, config: DcpConfig): void
//#endregion
